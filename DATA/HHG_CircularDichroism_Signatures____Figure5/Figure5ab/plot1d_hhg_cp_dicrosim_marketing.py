...
if iparam=='Trivial':
    plt.text(0.5, 5e-2, '(a) Triv. Phase', fontsize=37);
...
#plt.text(6.0, 1e-4, r'$ {\rm CD} = \,\frac{ RCP - LCP}{ RCP + LCP} = +1$'
#         , fontsize=40, color="w");
...
#plt.xlabel(r'$\rm \omega/\omega_0$', fontsize=34 ,color='w');
#plt.ylabel(r'$\rm I_{HHG} \, Emission$ ', fontsize=34 );
#plt.ylabel(r'$\rm Emission$ ', fontsize=34 ,color='w');
...
plt.tick_params(labelsize = 30 ,color='w');

#yticks0  = [1e-16, 1e-12, 1e-8, 1e-4, 1e0, 1e4 ]#10., 15, 5.0];
yticks0  = [1e-16,  1e-5,  1e0, 1e4 ]#10., 15, 5.0];
plt.yticks( yticks0 ,color='w');
plt.ylim([0.98e-16,1.0e-2])

ax.set_facecolor((.1, .1, .1) );
#ax.set_facecolor('#eafff10');


if iparam=='Trivial':
    fname='LCP_Vs_RCP_Trivial.ps'
else:
