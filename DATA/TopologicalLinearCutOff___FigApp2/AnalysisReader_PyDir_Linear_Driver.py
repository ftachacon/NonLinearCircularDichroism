...
xSinter         = abs( xFFT_Jinter[Nthalf:Nt-1] )**2 + hzero ;
NewNt           = len(xSinter);


ySinter         = abs( yFFT_Jinter[Nthalf:Nt-1] )**2 + hzero ;
Sinter          = abs( xFFT_Jinter[Nthalf:Nt-1] )**2 + abs( yFFT_Jinter[Nthalf:Nt-1] )**2 + hzero ;


xSintra         =  abs( xFFT_Jintra[Nthalf:Nt-1] )**2 + hzero ;
ySintra         =  abs( yFFT_Jintra[Nthalf:Nt-1] )**2 + hzero ;
Sintra 	        =  abs( xFFT_Jintra[Nthalf:Nt-1] )**2 + abs( yFFT_Jintra[Nthalf:Nt-1] )**2 + hzero;

xSpectrum       = abs( xFullRadiation[Nthalf:Nt-1] )**2 + hzero ;
ySpectrum       = abs( yFullRadiation[Nthalf:Nt-1] )**2 + hzero ;
Spectrum        = abs( xFullRadiation[Nthalf:Nt-1] )**2 + abs( yFullRadiation[Nthalf:Nt-1] )**2 + hzero ;


a_dJ_p          =  abs( dJ_p )**2 + hzero ;
a_dJ_m          =  abs( dJ_m )**2 + hzero ;
#####################################################
...
xSinter.shape     = (NewNt,1)
ySinter.shape     = (NewNt,1)
xSintra.shape     = (NewNt,1)
ySintra.shape     = (NewNt,1)
xSpectrum.shape     = (NewNt,1)
ySpectrum.shape     = (NewNt,1)
Spectrum.shape      = (NewNt,1)
...
temp0           = np.array([dt]);
temp0.shape     = (1,1)
spectrum_output = np.concatenate( ( temp0, Spectrum  ),axis=0 );
...
#################################
# INTER AND INTRA BAND CURRENTS
########

temp1           = np.array([Phi0]);
temp1.shape     = (1,1)
real_jxoutput   = np.concatenate( ( temp1, xSinter ),axis=0 );


temp1           = np.array([M0t2]);
temp1.shape     = (1,1)
imag_jxoutput    = np.concatenate( ( temp1, xSintra ),axis=0 );



temp1           = np.array([ChernNo]);
temp1.shape     = (1,1)
real_jyoutput   = np.concatenate( ( temp1, ySinter ),axis=0 );


temp1           = np.array([Eg]);
temp1.shape     = (1,1)
imag_jyoutput   = np.concatenate( ( temp1, ySintra ),axis=0 );


temp1           = np.array([dt]);
temp1.shape     = (1,1)
spectrum_output = np.concatenate( ( temp1, Spectrum  ),axis=0 );


OutputData2      = w_output;
OutputData2      = np.concatenate( (OutputData2, real_jxoutput   ), axis=1 );
OutputData2      = np.concatenate( (OutputData2, imag_jxoutput   ), axis=1 );
OutputData2      = np.concatenate( (OutputData2, real_jyoutput   ), axis=1 );
OutputData2      = np.concatenate( (OutputData2, imag_jyoutput   ), axis=1 );
OutputData2      = np.concatenate( (OutputData2, spectrum_output ), axis=1 );

ofname          = "/HHG__INTER__INTRA__Phi0__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2) + "__e0__" + str("%.3f"%ellip) +"__E0" +str("%.4f"%E0) + "__.dat"

out                = ProjPath + ofname
np.savetxt(out, OutputData2 , fmt='%1.16e')
...
p11, = plt.plot( w[Nthalf:Nt-1]/w0, Sinter, 'b',  lw = 1.2, label='$J_{er}$' );
p22, = plt.plot( w[Nthalf:Nt-1]/w0, Sintra, 'r',  lw = 1.2, label='$J_{ra}$' );
plt.legend([p11, p22], [ '$J_{er}$', '$J_{ra}$'],fontsize=18);
plt.xlabel(r'$\rm Harmonic-Order$', fontsize=30 );
plt.ylabel(r'$\rm I_{HHG}\, (arb. u.)$', fontsize=30 );
plt.tick_params(labelsize = 28 );
xticks0  = np.arange(1,50,4);
plt.xticks( xticks0 );
yticks0  = [-20., -15., -10., -5., 0., 5. ];
plt.yscale('log')
plt.yticks( np.power(10,yticks0) );
plt.grid(True);

xaxmin      = 0;        # controling harmonic-order axis limits, down
xaxmax      = 15;       # controling harmonic-order axis limits, u
...
p3,  = plt.plot( w[Nthalf:Nt-1]/w0, Spectrum, 'g',  lw = 1.2, label='$J_{t}$' );
