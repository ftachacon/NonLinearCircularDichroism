...
N_MPI_TASKs=47 #2   #No. of MPI task or cores for parallelization
...
Nx=201 #401   #201 #201 #40#             # No. of points along kx
Ny=235 #465   #233 #233 #117#            # No. of points along ky, ratio Nx/Ny=1.72 or, dpending on box 1.16
...
Ncycles=16         # No. of Opt. cycles
#ellip=-1.0         # Ellipticity

dt=1.0             # Time-Steps (a.u.)
...
date
ls -lh exe*
time -p mpirun -n ${N_MPI_TASKs} exec_hhg_mpi ${phi0}  ${Mt2}  ${Nx}  ${Ny}  ${E0}  ${Ncycles}  ${dt}  ${dephasing} ${ellip} ${eps} ${gauge} ${rflag} ${gbox_ky_down} ${gbox_ky_up} ${fbz_shift} ${ky_shift_down} ${diag} ${ksfactor}
