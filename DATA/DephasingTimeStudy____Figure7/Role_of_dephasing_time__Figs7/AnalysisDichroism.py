...
iparam 	    	=  float(set_of_params[1]);  #mag. flux phase param
jparam 	    	=  float(set_of_params[2]);  #dephasing param
T2          = jparam;
...
RPol        = 'HHG__Phi0__'+ str("%.3f"%iparam) + '__M0t2__2.540__e0__-1.000__T2__' + str('%.3f'%T2) +'__.dat';
LPol        = 'HHG__Phi0__'+ str("%.3f"%iparam) + '__M0t2__2.540__e0__1.000__T2__'  + str('%.3f'%T2) +'__.dat';
...
phase_params=[]
phase_params.append(Phi0)
phase_params.append(M0t2)
phase_params.append(ChernNo)
phase_params.append(Eg)


ofname          = "/Haldane__Phase__Params__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2) + "__T2__"  + str('%.3f'%T2) +".dat"
out                = ProjPath + ofname;

np.savetxt( out, phase_params, fmt='%1.16f');
...
print "L_DJ_m.shape ", L_DJ_m.shape
print "L_DJ_p.shape ", L_DJ_p.shape
print "Nt = ",Nt
...
leffict = np.zeros((Nf,1)); 
reffict = np.zeros((Nf,1)); 

for n in range(0,Nf):
    
    
    hint    =  horder[n,0] - 1./1.5
    hfinal  =  horder[n,0] + 1./1.5
    
    ind0    = int( ( hint - w[0] )/dw );
    indf    = int( ( hfinal - w[0] )/dw );
    
    if n==0: 
        rtemp1 = np.sum( R_DJ_p[ind0:indf] + R_DJ_m[ind0:indf]  );
        ltemp1 = np.sum( L_DJ_p[ind0:indf] + L_DJ_m[ind0:indf]  );
        
    print ("i0 = ", ind0, "; if = ", indf, "; hint = ", hint, "; hf = ", hfinal )
    ...
    rtempH0 = np.sum( R_DJ_p[ind0:indf] + R_DJ_m[ind0:indf]  );
    ltempH0 = np.sum( L_DJ_p[ind0:indf] + L_DJ_m[ind0:indf]  );    
    
    reffict[n] = rtempH0/rtemp1 
    leffict[n] = ltempH0/ltemp1 
...
OutputData  = np.concatenate( (OutputData, reffict  ), axis=1 );
OutputData  = np.concatenate( (OutputData, leffict  ), axis=1 );

ofname          = "/Dichroism__Phi0__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2) + '__T2__'  + str('%.3f'%T2) + "__.dat"
...
xaxmax      = 29.7;   # controling harmonic-order axis limits, u
...
fname='/LeftRightRotations'+ str("%.2f"%ChernNo) + "__Phi0__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2)  + '.pdf'
...
fname='/CD_On_HHG_CNo'+ str("%.2f"%ChernNo) + "__Phi0__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2)  + '.pdf'
...
plt.plot( horder, leffict, 'ro', lw = 2., label='$J_{x}$' );
plt.plot( horder, reffict, 'bo', lw = 2., label='$J_{x}$' );

plt.xlabel(r'$\rm Harmonic-Order$', fontsize=30 );
plt.ylabel(r'$\rm efficience $', fontsize=30 );
plt.tick_params(labelsize = 28 );
...
fname='/Ratio_On_HHG_CNo'+ str("%.2f"%ChernNo) + "__Phi0__" + str("%.3f"%Phi0) + "__M0t2__" + str("%.3f"%M0t2)  + '.pdf'
